...
DynNom.Glm <- function(model, data, clevel = 0.95, m.summary = c("raw", "formatted"),
  ...
  runApp(list(
    ...
    server = function(input, output){
      ...
      neededVar <- vars[-1]
      data <- cbind(model$y[1:length(model$y)],na.omit(data[, neededVar]))
      ...
      if (model$family$family == "poisson" |
        ...
        output$plot <- renderPlotly({
          ...
          if (dim(input.data)[1] > 11)
            yli <- c(dim(input.data)[1] - 11.5, dim(input.data)[1] - 0.5)
          p <- ggplot(data = data2()[!is.na(data2()$Prediction),], aes(x = Prediction, y = PredictNO, text = Covariates,
                                                                       label = Prediction, label2 = Lower.bound, label3=Upper.bound)) +
            geom_point(size = 2, colour = data2()$count[!is.na(data2()$Prediction)], shape = 15) +
            ylim(yli[1], yli[2]) + coord_cartesian(xlim = lim) +
            geom_errorbarh(xmax = data2()$Upper.bound[!is.na(data2()$Prediction)], xmin = data2()$Lower.bound[!is.na(data2()$Prediction)],
                           size = 1.45, height = 0.4, colour = data2()$count[!is.na(data2()$Prediction)]) +
            labs(title = paste(clevel * 100, "% ", "Confidence Interval for Response", sep = ""),
                 x = "Response", y = NULL) + theme_bw() +
